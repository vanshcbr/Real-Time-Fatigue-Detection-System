...
pygame.mixer.init()
pygame.mixer.music.load("alarm.wav")  # Load alarm sound file
...
def calculate_mar(mouth_landmarks):
    A = dist.euclidean(mouth_landmarks[1], mouth_landmarks[5])  # Vertical distance
    B = dist.euclidean(mouth_landmarks[2], mouth_landmarks[4])  # Vertical distance
    C = dist.euclidean(mouth_landmarks[0], mouth_landmarks[3])  # Horizontal distance
    MAR = (A + B) / (2.0 * C)
    return MAR
...
MAR_THRESH = 0.3  # Initial threshold for MAR to detect yawning
...
YAWN_DURATION_THRESH = 1.5  # Reduced yawning duration threshold (in seconds)
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            if BASELINE_MAR is None:
                ...
                dynamic_mar_thresh = BASELINE_MAR * 1.5  # Adjust threshold dynamically
